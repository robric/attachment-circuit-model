...
Bearer:
: Refers to the properties of a link that connects a customer node/site to a provider network. These properties are generally below Layer 3 and cover mostly Layer 1 and/or Layer 2. A bearer can be a wireless or wired link. One or multiple technologies can be used to build a bearer. The bearer type can be specified by a customer.
: A reference to uniquely identify a bearer within a provider network is generated by the operator (e.g., customer line identifier). Such a reference can be retrieved by a customer and used in subsequent service placement requests to unambiguously identify where a service is to be bound.
: The concept of bearer can be generalized to refer to the perquisite underlying setup for the activation of an attachment circuit. The same bearer may host one or multiple attachment circuits.
