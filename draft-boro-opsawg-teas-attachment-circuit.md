...
: The concept of bearer can be generalized to refer to the required underlying connection for the provisioning of an attachment circuit. The same bearer may host one or multiple attachment circuits (e.g., multiple VLANs on the same bearer that is provided by a phyical link).
