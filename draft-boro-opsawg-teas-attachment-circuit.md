...
Bearer:
: Refers to the properties of an attachment that connects a customer node/site to a provider network. These properties are generally below Layer 3 (that is, they cover Layer 1 and Layer 2). A bearer can be a wireless or wired link. One or multiple technologies can be used to build a bearer. The bearer type can be specified by a customer.
: A reference to uniquely identify a bearer within a provider network is generated by the operator. Such a reference can be retrieved by a customer and used in subsequent service placement requests to unambiguously identify where a service is to be bound.
