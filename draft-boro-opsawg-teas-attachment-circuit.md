...
: A bearer reference to uniquely identify a bearer within a provider network is generated by the operator (e.g., customer line identifier). Such a reference can be retrieved by a customer and used in subsequent service placement requests to unambiguously identify where a service is to be bound.
